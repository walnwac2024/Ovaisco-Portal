        ...
        <div className="p-6 max-w-6xl mx-auto space-y-6">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div className="lg:col-span-8 space-y-6">
                    ...
                    <div className="bg-white rounded-3xl border border-slate-100 p-6 flex items-center gap-6 shadow-sm">
                        <div className="w-16 h-16 bg-slate-50 rounded-2xl flex items-center justify-center text-customRed font-black text-2xl border border-slate-100 shadow-inner">
                            {data.name?.charAt(0)}
                        </div>
                        <div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-6">
                                {allowances.length > 0 ? allowances.map((item, idx) => (
                                    <div key={idx} className="flex justify-between items-center group">
                                        <span className="text-slate-500 font-bold group-hover:text-slate-900 transition-colors">{item.label}</span>
                                        <span className="text-slate-900 font-black transition-all transform group-hover:text-customRed">Rs. {Number(item.value).toLocaleString()}</span>
                                    </div>
                                )) : (
                    ...
                    <div className="bg-white rounded-[2.5rem] border border-slate-100 shadow-sm overflow-hidden">
                        <div className="px-8 py-6 bg-customRed/5 border-b border-slate-100 flex justify-between items-center">
                        ...
                        <div className="p-10 space-y-8">
                            {/* Absent Info */}
                            <div className="flex items-start gap-6 p-6 bg-slate-50/50 rounded-3xl border border-slate-100 transition-all hover:border-customRed/20 hover:bg-white group">
                                <div className="w-14 h-14 bg-white rounded-2xl shadow-sm flex items-center justify-center text-customRed group-hover:bg-gradient-to-br group-hover:from-customRed group-hover:to-red-600 group-hover:text-white transition-all duration-300 border border-slate-100">
                            ...
                            {/* Late Info */}
                            <div className="flex items-start gap-6 p-6 bg-slate-50/50 rounded-3xl border border-slate-100 transition-all hover:border-amber-200 hover:bg-white group">
                                <div className="w-14 h-14 bg-white rounded-2xl shadow-sm flex items-center justify-center text-amber-500 group-hover:bg-amber-500 group-hover:text-white transition-all duration-300 border border-slate-100">
                            ...
                            <div className="pt-10 space-y-6">
                                <div className="p-6 bg-white/5 rounded-3xl border border-white/10 backdrop-blur-md">
                                    <p className="text-[10px] text-slate-400 leading-relaxed font-bold uppercase tracking-wide opacity-60 text-center">
                                        ...
                                        Generated by <span className="text-slate-200">HRM Pro</span> on {new Date(data.created_at).toLocaleDateString()}.
                                    </p>
                                </div>
